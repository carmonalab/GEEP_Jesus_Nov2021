...
#remotes::install_github("ncborcherding/scRepertoire",ref="3f676c5beca96878585811ed557813dafd241e48") # version 1.3.2
...
data.seurat.CD8.projected <- make.projection(data.seurat.CD8,ref = ref,filter.cells = F, batch_correction = 0.1)
plot.projection(ref,data.seurat.CD8.projected, pointsize = 0.3, linesize = 0.5)
...
as.data.frame(sort(table(data.seurat.CD8$functional.cluster), decreasing = T))
...
Recalculate embeddings with new projected data (focus on C4 and C5)
```{r}
palette <- c("#edbe2a", "#A58AFF", "#53B400", "#F8766D", "#00B6EB", "#d1cfcc", "#FF0000", "#87f6a5", "#e812dd")
names(palette) <- levels(ref$functional.cluster)

which.clusters <- c('C4','C5') 
proj.sub <- subset(data.seurat.CD8.projected, subset=cluster.lab %in% which.clusters)

merged <- recalculate.embeddings(ref=ref, projected=proj.sub, ndim=15, recalc.pca = T, umap.method = 'umap', resol = 0.8)

Idents(merged) <- "functional.cluster"
plot.projection(ref=merged, query=subset(merged, subset=ref_or_query=="query"), cols = palette, linesize=0.5) + theme(aspect.ratio = 1)
```

A different visualization
```{r fig.width=4}
a <- DimPlot(merged, reduction = "umap", cols=palette) + theme(aspect.ratio=1) + 
  theme(axis.ticks = element_blank(), axis.text = element_blank()) + NoLegend()
sub <- merged
sub$functional.cluster[sub$ref_or_query == "query"] = NA
b <- DimPlot(sub, reduction = "umap", cols=palette, 
             na.value = 'grey95', group.by = "functional.cluster") + 
  theme(aspect.ratio=1, axis.ticks = element_blank(), axis.text = element_blank()) + NoLegend() + ggtitle("Reference")

sub <- merged
sub$functional.cluster[sub$ref_or_query == "ref"] = NA
c <- DimPlot(sub, reduction = "umap", cols=clusterColors[which.clusters], 
             group.by = "cluster.lab", na.value = 'grey95') + 
  theme(aspect.ratio=1, axis.ticks = element_blank(), axis.text = element_blank()) + ggtitle("Projected")

d <- FeaturePlot(sub, features="newclusters") + theme(aspect.ratio = 1)
...
# Differentially expressed genes between C4 and C5
```{r}
library(MAST)
data.use <- data.seurat.CD8.projected.Tex
#data.use <- data.seurat.CD8.projected

Idents(data.use) <- "cluster.lab"
markers4vs5 <- FindMarkers(data.use, ident.1 = "C5",ident.2 = "C4",
                           test.use = "MAST", min.diff.pct = 0.01,
                           logfc.threshold = 0.25, min.pct=0.05, assay="RNA")
...
#Save to file
write.csv(up.c4, file="DEG.upC4.csv")
write.csv(up.c5, file="DEG.upC5.csv")
